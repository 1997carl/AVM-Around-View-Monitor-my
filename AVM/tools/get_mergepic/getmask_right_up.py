...
def main():
    ...
    # 读取原始图片
    front = cv2.imread(
        "D:/mypython/works/get_mask_and _weights/source/400x400/right_up_f.jpg"
    )
    left = cv2.imread(
        "D:/mypython/works/get_mask_and _weights/source/400x400/right_up_r.jpg"
    )
    # 获取图片尺寸信息
    ...
    polyB = np.array(
        [[400, 300], [400, 400], [0, 400]], dtype=np.int32
    )  # 侧边视图的非重叠区域大小
    polyA = np.array(
        [[100, 0], [0, 0], [0, 400]], dtype=np.int32
    )  # 最后一个点往往设置重合
    polygon_points = []
    ...
    Image.fromarray((G * 255).astype(np.uint8)).save(
        "D:\\mypython\\works\\get_mask_and _weights\\output\\weights_right_up.png"
    )
