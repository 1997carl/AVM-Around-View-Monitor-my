...
def get_overlap_region_mask(
    imA, imB
):  # 输入相同大小的图片计算重叠的区域，输出重叠部分的掩码
    """
    ...
    mask = cv2.dilate(
        mask, np.ones((2, 2), np.uint8), iterations=2
    )  # 在这个上下文中，膨胀操作通常用于扩展掩码中的白色区域，以便更好地捕捉和处理重叠的区域。
    return mask
...
def main():
    ...
    # 读取原始图片
    front = cv2.imread(
        "D:/mypython/works/get_mask_and _weights/source/400x400/right_down_b.jpg"
    )
    left = cv2.imread(
        "D:/mypython/works/get_mask_and _weights/source/400x400/right_down_r.jpg"
    )
    # 获取图片尺寸信息
    ...
    ###########直接传入重叠区域的图片##########
    front_overlap_left = cv2.imread(
        "D:/mypython/works/get_mask_and _weights/source/400x400/right_down_b.jpg"
    )
    left_overlap_front = cv2.imread(
        "D:/mypython/works/get_mask_and _weights/source/400x400/right_down_r.jpg"
    )
    # step2 生成边界
    # 2.1图片中重叠的掩码区域
    left_up_overlap = get_overlap_region_mask(
        front_overlap_left, left_overlap_front
    )  # 重叠区域掩码 用于后续布尔运算得到边界区域
    left_up_overlapInv = cv2.bitwise_not(left_up_overlap)  # 非重叠部分的掩码
    front_overlap_left_diff = cv2.bitwise_and(
        front_overlap_left, front_overlap_left, mask=left_up_overlapInv
    )  # 图像a非重叠的部分
    left_overlap_front_diff = cv2.bitwise_and(
        left_overlap_front, left_overlap_front, mask=left_up_overlapInv
    )  # 图像b非重叠的部分
    ...
    polyB = np.array(
        [[400, 0], [400, 100], [0, 0]], dtype=np.int32
    )  # 侧边视图的非重叠区域大小
    polyA = np.array(
        [[0, 400], [100, 400], [0, 0]], dtype=np.int32
    )  # 最后一个点往往设置重合
    polygon_points = []
    for y in range(height):
        for x in range(width):
            if (
                cv2.pointPolygonTest(polyB, (x, y), False) < 0
                and cv2.pointPolygonTest(polyB, (x, y), False) < 0
            ):
    ...
    #     Image.fromarray((G * 255).astype(np.uint8)).save("D:/mypython/works/get_mask_and _weights/output/weights_left_down.png")#保存权重矩阵

    #     #Image.fromarray(left_up_overlap.astype(np.uint8)).save("D:/mypython/works/get_mask_and _weights/output/masks_change.png")#掩码矩阵
    #     print("loadng{}%,y:{},x:{},dis{},G[y, x]:{}".format(num_for/len(polygon_points)*100,y,x,distToB,G[y, x]))

    # 定义处理每个点的函数
    def process_point(y, x):
        ...
        if distToB < dist_threshold:
            distToB *= distToB
            distToA *= distToA
            return y, x, distToB / (distToA + distToB)
        else:
            return None

    # 使用多线程并行处理所有点
    with ThreadPoolExecutor() as executor:
        results = list(executor.map(lambda args: process_point(*args), polygon_points))
    ...
    Image.fromarray((G * 255).astype(np.uint8)).save(
        "D:\\mypython\\works\\get_mask_and _weights\\output\\weights_right_down.png"
    )

    # 输出完成后的信息
    print("Loading 100%, Process complete")
