...
def main():
    #STEP1 裁剪待融合区域的两幅图片
    #读取原始图片
    front = cv2.imread('D:/mypython/works/get_mask_and _weights/source/400x400/left_down_b.jpg')
    left=cv2.imread('D:/mypython/works/get_mask_and _weights/source/400x400/left_down_l.jpg')
    #获取图片尺寸信息
    ...
    ###########直接传入重叠区域的图片##########
    front_overlap_left = cv2.imread('D:/mypython/works/get_mask_and _weights/source/400x400/left_down_b.jpg')
    left_overlap_front=cv2.imread('D:/mypython/works/get_mask_and _weights/source/400x400/left_down_l.jpg')
    #step2 生成边界
    #2.1图片中重叠的掩码区域
...
#########################################结束绘制边界######################
    ...
    polyB=np.array([[0, 100], [0, 0], [400, 0]], dtype=np.int32)#侧边视图的非重叠区域大小
    polyA=np.array([[300, 400], [400, 400], [400, 0]], dtype=np.int32)                    #最后一个点往往设置重合
    polygon_points = []
    ...
    # 保存权重矩阵图像
    Image.fromarray((G * 255).astype(np.uint8)).save("D:\\mypython\\works\\get_mask_and _weights\\output\\weights_left_down.png")
